...
3.  Six neighbourhoods- Bayview, Parkside, CCSF, Russian Hill, Tenderloin and Twin peaks
...
# Filter the data for Tenderloin neighborhood and years 2010-2018
filtered_data_tenderloin <- mutate_data |>
  filter(city == "san francisco", room_in_apt == 0, nhood == "tenderloin", year >= 2010, year <= 2018)

# Filter the data for Twin Peaks neighborhood and years 2010-2018
filtered_data_twinpeaks <- mutate_data |>
  filter(city == "san francisco", room_in_apt == 0, nhood == "twin peaks", year >= 2010, year <= 2018)

```

For ease of viewing the trends, i have calculated the median price for each of the area using the following:
...
summary_stats_tenderloin <- filtered_data_tenderloin |>
  group_by(year) |>    #Grouping by year
  summarize(median_price_tenderloin = median(price_per_bedroom, na.rm = TRUE))
...
Scatter plot (Jitter) of each of the neighborhood highlighting the trends in rent per bedroom:

1.  Bayview neighbourhood:
